...
const categoryConfig: Record<PostCategory, { label: string; emoji: string }> = {
  free: { label: "냥냥 잡담", emoji: "🐱" },
  tip: { label: "고양이 도서관", emoji: "📚" },
  gallery: { label: "모험가 갤러리", emoji: "🖼️" },
  qna: { label: "질문해냥", emoji: "❓" },
};
...
export default function PostContent({ post }: { post: Post }) {
  ...
  const handleComment = async () => {
    ...
    if (error) {
      toast("댓글 실패: " + error.message, "error");
    } else {
      setCommentText("");
      loadComments();
    }
  ...
  const handleDeleteComment = async (commentId: string) => {
    if (!confirm("댓글을 삭제할까냥?")) return;
    await supabase.from("comments").delete().eq("id", commentId);
    loadComments();
  };

  const handleDeletePost = async () => {
    if (!confirm("이 글을 삭제할까냥? 되돌릴 수 없다냥!")) return;
